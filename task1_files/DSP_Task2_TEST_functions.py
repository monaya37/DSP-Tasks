...
def AddSignalSamplesAreEqual(userFirstSignal, userSecondSignal, Your_indices, Your_samples):
    if (userFirstSignal == 'task1_files\\Signal1.txt' and userSecondSignal == 'task1_files\\Signal2.txt'):
        file_name = "task1_files\\add.txt"  # write here the path of the add output file
...
def SubSignalSamplesAreEqual(userFirstSignal, userSecondSignal, Your_indices, Your_samples):
    if (userFirstSignal == 'task1_files\\Signal1.txt' and userSecondSignal == 'task1_files\\Signal2.txt'):
        file_name = "task1_files\\subtract.txt"  # write here the path of the subtract output file
...
def MultiplySignalByConst(User_Const, Your_indices, Your_samples):
    if (User_Const == 5):
        file_name = "task1_files\\mul5.txt"  # write here the path of the mul5 output file
...
def ShiftSignalByConst(Shift_value, Your_indices, Your_samples):
    if (Shift_value == 3):  # x(n+k)
        file_name = "task1_files\\advance3.txt"  # write here the path of delay3 output file
    elif (Shift_value == -3):  # x(n-k)
        file_name = "task1_files\\delay3.txt"  # write here the path of advance3 output file
...
def Folding(Your_indices, Your_samples):
    file_name = "task1_files\\folding.txt"  # write here the path of the folding output file
    expected_indices, expected_samples = ReadSignalFile(file_name)
    if (len(expected_samples) != len(Your_samples)) and (len(expected_indices) != len(Your_indices)):
